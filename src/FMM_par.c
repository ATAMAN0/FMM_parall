...
#define CHUNK 20
static long num_steps = 16000;double step;
...
------------------------------------------------------------------------------*/
  ...
  /* Scan particles to add their multipoles */
    // num_threads (NUM_THREADS)
      // #pragma omp  for private(c)  schedule (dynamic,CHUNK)  
  for (j=0; j<Npar; j++) {
    //  #pragma omp parallel for  reduction(+:fop1,c) schedule (dynamic,CHUNK)  
  ...
  #pragma omp parallel
{
   ...
   #pragma omp  for  reduction(+:eng,fop1)   schedule (dynamic,CHUNK)  //num_threads (NUM_THREADS)
  for (j=0; j<Npar-1; j++){
    eng += q[j]*pot[j];
...
void all_direct() {
/*------------------------------------------------------------------------------
  All-pair  calculation of the electrostatic potential.
------------------------------------------------------------------------------*/
  int j,k,a;
  ...
  for (j=0; j<Npar; j++)
   ...
   #pragma omp parallel  for reduction (+:pot_direct)    schedule (dynamic,CHUNK) 
  for (j=0; j<Npar-1; j++)
{
    #pragma omp parallel  for reduction (+:rjk,fop2)   schedule (dynamic,CHUNK)  
    for (k=j+1; k<Npar; k++) {
        rjk=0.0;
        rjk += (z[j][0]-z[k][0])*(z[j][0]-z[k][0]); 
        rjk += (z[j][1]-z[k][1])*(z[j][1]-z[k][1]);
		    fop2 += 3*2;
...
{
     omp_set_num_threads(NUM_THREADS);
 #pragma omp parallel for  reduction(+:eng_direct,fop2)   schedule (dynamic,CHUNK)  // num_threads (NUM_THREADS)
  for (j=0; j<Npar-1; j++){
